 ...
 * (P/P) in Ts" line, with T in seconds or milliseconds. Undefined when the log has none.
 */
export function staticGeneration(log: string): StaticGeneration | undefined {
  const matches = [...log.matchAll(/Generating static pages using (\d+) workers \((\d+)\/\2\) in ([\d.]+)(ms|s)\b/g)]
  const last = matches.at(-1)
  if (!last) return undefined
  const value = Number(last[3])
  return { workers: Number(last[1]), pages: Number(last[2]), seconds: last[4] === 'ms' ? value / 1000 : value }
}
