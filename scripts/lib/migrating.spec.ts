...
describe('migratingFile', () => {
  it('records its source and points README links at the line’s pages', () => {
    const file = migratingFile(
      'See [Guards](../README.md#guards), [the README](../README.md) and [elsewhere](../README.md#gone).',
      {
        line: '4.1',
...
describe('fetchMigrating', () => {
  it('reads the guide at the attested commit, and reports a failure', async () => {
    ...
    const fetchImpl = async (url: string) => (
      seen.push(url),
      url.includes('good') ? new Response('# Guide') : new Response('', { status: 404 })
    )
