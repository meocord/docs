...
describe('once the site is indexable', () => {
  ...
  it('drops the noindex header and opens robots.txt and the sitemap', async () => {
    ...
    expect(await robotsResponse.text()).toContain('Sitemap: https://meocord.meoverse.com/sitemap.xml')
    expect(await sitemap().text()).toContain('<loc>https://meocord.meoverse.com/</loc>')
  })
})
